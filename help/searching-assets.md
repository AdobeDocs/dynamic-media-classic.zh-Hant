...
要對顯示在右側「瀏覽面板」中的資料夾或搜索結果的內容進行排序，請在全局導航欄上按一下&#x200B;**[!UICONTROL 排序]**，然後選擇一個選項。 選項包括&#x200B;**[!UICONTROL Name]**、**[!UICONTROL Size(KB)]**、**[!UICONTROL Type]**、**[!UICONTROL Date Created]**&#x200B;和&#x200B;**[!UICONTROL Last Modified]**。
...
使用「資產庫」中的「搜尋」欄位，以便執行簡單的搜尋。 您可以依據名稱來搜尋項目，或是搜尋中繼資料包含關鍵字的項目。

1. 在「資產庫」的&#x200B;**[!UICONTROL 「資料夾」]**&#x200B;面板中，選擇要在特定資料夾及其子資料夾中搜索的資料夾。
1. 在「資產資料庫」的「搜尋」欄位左側，按一下&#x200B;**[!UICONTROL 放大鏡]**&#x200B;圖示以開啟下拉式清單。
1. 在下拉式清單中，選擇一個選項，說明您希望搜尋的範圍或範圍。 您可以選擇「所有檔案和資料夾內」]**、「選定資料夾內」**[!UICONTROL &#x200B;或「選定資料夾和子資料夾內」]**或「**[!UICONTROL 」。****
1. 在「搜尋」欄位中輸入搜尋詞。
1. 在「搜索」欄位的右側，按一下&#x200B;**[!UICONTROL Go]**&#x200B;或按&#x200B;**[!UICONTROL Enter]**。
...
在「資產庫」的「搜尋」欄位正下方，按一下「**[!UICONTROL 進階搜尋]**」以使用許多條件進行搜尋，包括中繼資料欄位中的值。

在進階搜尋中指定下列任一條件：

* **依資產類型篩選**  — 只需在功能表上選擇資產類型，即可將搜尋範圍縮小至一個資產類型。
...
* **搜尋條件**  — 建立一或多個搜尋欄位以搜尋中繼資料。建立搜尋欄位:
   ...
   1. 在「高級搜索」中，在&#x200B;**[!UICONTROL 搜索標準]**&#x200B;標題下，在&#x200B;**[!UICONTROL 添加欄位]**&#x200B;菜單的左側，按一下向下三角形箭頭表徵圖以開啟下拉清單。 選擇中繼資料檢視。 您可以選擇&#x200B;**[!UICONTROL 具有值]**、**[!UICONTROL 緊密視圖]**、**[!UICONTROL IPTC]**、**[!UICONTROL 元資料伺服器發佈欄位]**&#x200B;或&#x200B;**[!UICONTROL XMP]**&#x200B;的所有屬性。
   1. 按一下&#x200B;**[!UICONTROL 新增欄位]**&#x200B;下拉式功能表，然後選擇欄位名稱。
...
按一下&#x200B;**[!UICONTROL 移除搜尋欄位]**&#x200B;圖示（內有&quot;X&quot;的圓圈），即可刪除搜尋欄位。

在「進階搜尋」面板的右下角，按一下「**[!UICONTROL 搜尋]**」以開始搜尋。 搜尋結果會顯示在右側的「瀏覽面板」中。 您可以更改任何搜索條件，然後按一下&#x200B;**[!UICONTROL Search]**&#x200B;以再次運行搜索。

按一下&#x200B;**[!UICONTROL 清除]**&#x200B;以清除搜索標準並開始新搜索。 完成搜索後，按一下&#x200B;**[!UICONTROL 關閉]**&#x200B;以關閉「搜索」面板。

## 使用中繼資料篩選資產 {#filter-assets-using-metadata}

您可以在資產庫的「篩選」標籤中篩選資產。若要篩選資產，您可以使用中繼資料值做為條件。選擇要篩選的中繼資料欄位後，「篩選」索引標籤會列出您在所選欄位中輸入的所有中繼資料值。 也會列出每個值所指派的資產數量。 例如，在「建立者」元資料欄位上的篩選操作中，「篩選器」索引標籤會列出在「建立者」元資料欄位中針對不同資產輸入的所有名稱。 也會列出每個名稱的資產數量，以及指派名稱的資產數量。 接著按一下中繼資料值，即可查看曾指定過該值的所有資產。在示例中，按一下Prairie Cat元資料值，查看在「建立者」元資料欄位中輸入了名稱Prairie Cat的所有資產。 您可以使用多個中繼資料欄位作為篩選條件，來進行篩選。
...
1. 在每個面板中，選取要篩選的中繼資料值。每個面板中不能選取多個中繼資料值。
   ...
   >[!NOTE]
   >
   >若要暫時從篩選操作中移除欄位，請按一下「取消選取全部&#x200B;]**」。**[!UICONTROL &#x200B;此選項位於每個面板的頂端，且位於中繼資料值上方。

1. （可選）要保存篩選操作並能以後運行，請按一下「選擇預設集&#x200B;]**>**[!UICONTROL &#x200B;另存為新預設集&#x200B;]**」，然後在「**[!UICONTROL &#x200B;保存&#x200B;]**」對話框中輸入名稱。**[!UICONTROL 

### 儲存、重複和刪除篩選作業 {#saving-repeating-and-deleting-filter-operations}

請依照「篩選器」標籤上的下列指示操作，以便儲存、重複和刪除篩選器操作：

* **儲存篩選操作**  — 按一 **[!UICONTROL 下「選取預設]**  >  **[!UICONTROL 將目前儲存為新預設集」]**，然後在「儲存」對話方塊中輸 **** 入名稱。

* **重複篩選操作**  — 按一下 **[!UICONTROL 選]** 取預設，然後在功能表上選擇篩選操作的名稱。選單會列出您儲存的篩選作業。

* **從「選取預設集」功能表刪除篩選操作**  — 執行篩選操作。然後，按一下菜單上的「選擇預設集&#x200B;]**>**[!UICONTROL &#x200B;刪除預設集&#x200B;]**」。**[!UICONTROL 

## 使用中繼資料伺服器 {#using-the-metadata-server}

中繼資料伺服器是公用 API，可透過 http 要求依照中繼資料搜尋資產。

要配置元資料伺服器，請按一下「**[!UICONTROL 設定]** > **[!UICONTROL 應用程式設定]** > **[!UICONTROL 發佈設定]** > **[!UICONTROL 元資料伺服器]**」。

「中繼資料伺服器發佈」頁面可讓您設定下列選項：

* **即時發佈**  — 在進行中繼資料變更時自動推送這些變更，包括新資產、關鍵字變更等。

* **XMP封包**  — 發佈XMP封包。此封包不用於搜尋，但可提供最新的XMP。

* **關鍵字**  — 將關鍵字發佈至中繼資料伺服器以用於搜尋。

* **中繼資料伺服器發佈欄位**  — 選取要納入中繼資料的欄位。此選項可讓您判斷有多少資產的相關資訊可供公開使用。 「中繼資料檢視」也會顯示這些欄位，但是只能在中繼資料伺服器中予以變更。

按一下&#x200B;**[!UICONTROL 立即發佈]**&#x200B;以啟動作業。 會出現確認，告知您工作已經開始。
