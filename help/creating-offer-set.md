...
若為範本，請選取 **[!UICONTROL 新增和預覽]**，然後設定您選擇的參數。 其他選件集類型不包含參數，但您仍可以選取 **[!UICONTROL 預覽]** 和更改可用的預設集。
...
* **影像範本**  — 您可以在Adobe Dynamic Media Classic中使用 **[!UICONTROL 建置]** >範本基礎命令。 透過參數，可交換出及自訂範本的各個組件 (文字框中的文字、不同的影像)。對於提案集，舉例來說，您可以使用範本參數在提案集中的同一影像上建立變體。如需建立和參數化影像範本的相關資訊，請參閱 [建立範本參數](creating-template-parameters.md#creating_template_parameters).
...
1. 選取物件並選取 **[!UICONTROL 新增和預覽]**.

   頁面的左側列出了範本中的參數及參數值。

1. 變更參數值以建立提案。例如，在文字欄位中輸入不同文字、變更圖層大小、用一個影像交換另一個影像或者選擇不同的檢視器預設集。
1. 選擇 **[!UICONTROL 儲存]** 或 **[!UICONTROL 另存新檔**]** 將選件儲存為選件集的一部分。

   「Test&amp;Target選件集」頁面會列出您建立的選件。

1. 重複步驟 3 到 5 為提案集建立更多提案。
1. 完成後，在頁面右下角附近，請確定 **[!UICONTROL 儲存後發佈*]** 已選取（預設）。
1. 選擇 **[!UICONTROL 關閉]**，輸入選件集的名稱，然後選取 **[!UICONTROL 儲存]**.
...
1. 為選件集組合影像或影片。 在「Test&amp;Target選件集」畫面或「格線檢視」或「清單檢視」中開始，然後使用下列其中一種方法：
   ...
   * **網格視圖或清單視圖**  — 選取影像或影片，然後前往 **[!UICONTROL 建置]** > **[!UICONTROL Test&amp;Target選件集]**.

1. （可選）選擇影像或視頻並選擇 **[!UICONTROL 預覽]**. 在「預覽選件」頁面上，您可以變更所選影像或視訊的大小和外觀。 或者，您可以變更選件集中的所有影像或影片。

   * 選擇預設集以變更影像或視訊的外觀和大小。
   * 若要將您選擇的預設集套用至選件集中的所有選件，請選取 **[!UICONTROL 選擇全部預設集]** 框。

   選擇 **[!UICONTROL 儲存]** 儲存您對影像或影片選件的變更。 然後選取 **[!UICONTROL 關閉]** 返回「Test&amp;Target選件集」頁面。

1. 完成為選件集建立選件，並為不同影像選擇「影像預設集」後，請確定 **[!UICONTROL 儲存後發佈]** 已選取（預設）。
1. 選擇 **[!UICONTROL 儲存]** 並輸入選件集的名稱，然後選取 **[!UICONTROL 儲存]**.
...
1. 若要編輯優惠方案集，請在「格線檢視」或「清單檢視」中顯示優惠方案集，然後選取其 **[!UICONTROL 編輯]** 變換按鈕。
1. 在「Test&amp;Target選件集」頁面中，執行下列任一操作：

   * **移除選件**  — 選取選件，然後選取 **[!UICONTROL 刪除]** 從集中移除選件。
   * **新增優惠方案**  — 新增優惠方案的方式取決於您使用的優惠方案集類型：
      * **範本**  — 選擇 **[!UICONTROL 新增和預覽]**，並在新增和預覽選件頁面上建立其他選件。
...
1. 完成編輯時，在頁面右下角附近，請確定 **[!UICONTROL 儲存後發佈]** 已選取（預設）。
