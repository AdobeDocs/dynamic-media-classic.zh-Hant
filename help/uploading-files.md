...
下表列出AdobeDynamic Media Classic支援的資產檔案格式。 有關支援的Camera Raw檔案的資訊，請參閱[https://www.adobe.com/go/learn_s7_cameraraw_en](https://www.adobe.com/go/learn_s7_cameraraw_en)。
...
1. 在Adobe的Dynamic Media Classic案頭應用程式中，在全域導覽列上，按一下&#x200B;**[!UICONTROL Upload]**。
...
若要查看上載進度，按一下全域導覽列中的「**[!UICONTROL 工作]**」。您可以繼續使用Adobe Dynamic Media Classic，並隨時返回「工作」頁面，以檢閱進行中的工作。 若要取消進行中的上載工作，請選取「持續時間」旁邊的「**[!UICONTROL 取消]**」按鈕。

### 使用「透過FTP」標籤上傳檔案 {#upload-files-using-via-ftp}

1. 登入您特定地區專屬的AdobeDynamic Media Classic FTP站台。 使用管理員寄給您的 FTP 使用者名稱與密碼。
1. 在AdobeDynamic Media Classic中，在全域導覽列上按一下&#x200B;**[!UICONTROL Upload]**。
...
1. 在「上傳」頁面的右下角，按一下「**[!UICONTROL 提交上傳]**」。
   ...
   若要查看上載進度，請按一下全域導覽列中的「**[!UICONTROL 工作]**」。「工作」頁面隨即出現，並顯示上載的進度。您可以繼續使用Adobe Dynamic Media Classic，並隨時返回「工作」頁面，以檢閱進行中的工作。
...
* **作業**  — 按一 **** 下作業以選擇影響整個上載作業的選項。
   ...
   * **包含子資料夾**  — 只有在您選取了VIA FTP索引標籤 **[!UICONTROL 時才]** 可用。如果您要一併上載檔案夾及其子檔案夾，請選取此選項。您上傳的資料夾及其子資料夾名稱會在AdobeDynamic Media Classic中自動輸入。
...
* **色彩描述檔OPTIONS**  — 當您建立用於AdobeDynamic Media Classic動態傳送的最佳化檔案時，請選擇色彩轉換：
   ...
   * **保留原始色域**  — 在擷取至AdobeDynamic Media Classic時，保留原始色域而不進行任何顏色轉換。對於沒有內嵌色彩描述檔的影像，處理影像要求所需的任何色彩轉換，都是使用「發佈」設定中設定的預設色彩描述檔來完成。 這些顏色配置檔案不一定與使用此選項建立的檔案中的顏色一致。 因此，最好使用「預設色彩保存」選項。
