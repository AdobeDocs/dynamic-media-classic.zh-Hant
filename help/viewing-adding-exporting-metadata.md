...
* 用戶定義的欄位由其名稱標識，該名稱在「設定」>「應用程式設定」>「元資料」>「用戶定義」欄位中建立。 使用「產生」檔案功能，以正確的匯入格式取得所有已定義 UDF 的清單。
* XMP 中繼資料屬性的 (property-) 名稱前必須有相關的 XMP 字首。字首和名稱會以冒號區隔。您可以在「設定 > 應用程式設定 > 中繼資料 > 中繼資料結構編輯器」中找到 XMP 字首。您可以在相關的 XMP 資料結構文件中找到技術名稱。屬XMP性名稱不會出現在「生成檔案」功能中。
* 中繼資料結構 屬性的 (property-) 名稱前必須有相關的字首。字首和名稱會以冒號區隔。字首和屬性名稱皆是在「中繼資料結構編輯器」中定義。中繼資料結構屬性名稱不會顯示在「產生檔案」功能中。

例如: 關鍵字的 XMP 屬性是 XMP 資料結構「Dublin Core」，其字首為「dc」，而技術 XMP 名稱則是「subject」。字首和技術 XMP 名稱皆會合併至完整屬性名稱「dc:subject」。在 XML 中繼資料匯入格式中，「dc:subject」必須為屬性名稱。在以定位點分隔的匯入格式中，它必須是欄標題。
...
按一下全域導覽列上的「上傳」按鈕。 若要匯入中繼資料，請在「工作」畫面上選取「透過FTP ]**」標籤，然後按一下「工作選項****」。**[!UICONTROL &#x200B;在「上載作業選項」對話框中，選擇「處理元資料檔案」。
...
系統會自動為使用者定義欄位產生自訂資料結構，且其字首為「s7udf」。它是現有的用戶定義欄位，它們在自己的「設定」部分中進行編輯。
...
* 字首
   ...
   資料結構的技術唯一識別名稱。僅限字母a-z和A-Z。首碼在Dynamic Media經典UI中不可見，但當資產的中繼資料儲存在區塊和資料庫時XMP，會使用。 字首的用途為，在中繼資料伺服器上的中繼資料搜尋查詢中識別唯一的中繼資料欄位，或是進行匯入。
