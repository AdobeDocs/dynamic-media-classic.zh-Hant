...
您可以使用「應用程式設定」頁面來輸入一般設定、建立影像預設集、視訊編碼預設集、檢視器預設集，或定義預設檢視器和中繼資料。 您可以設定批次集預設集，以自動產生2D回轉集（例如）、發佈設定和視訊SEO設定。
...
* **發佈的伺服器名稱** -此伺服器是即時內容傳送網路(CDN)伺服器，用於您帳戶的特定系統產生的URL呼叫。除非Dynamic MediaClassic支援技術人員指示您更改此伺服器名稱，否則請勿更改此伺服器名稱。
...
* **Test&amp;Target伺服器名稱** -您的Test&amp;Target URL，最多及包含。com。如需有關取得此URL的指示，請參閱將[!DNL Dynamic Media Classic]與[!DNL Adobe Target Standard/Premium]整合。
...
* **CDN失效範本** -指定用於使CDN（內容傳送網路）快取失效的範本。
   ...
   在 URL 清單方塊中，按一下「**[!UICONTROL 繼續]**」以清除特定影像 URL 呼叫的快取。您也可以輸入或貼入URL清單方塊，以新增URL;您不需要事先設定範本。

   在您選取CDN失效範本並提出「使CDN無效」要求後，使用者介面中會彈出一個指標。 它可讓您估計清除快取所需的時間。

   同樣地，如果在Dynamic Media經典影像中，按一下「使CDN無效」時，選取了多個影像，則儲存的範本URL會參照每個影像。 ********&#x200B;因此，您可以定義CDN失效範本，此範本會參照網站上參考的每個URL（例如產品詳細資料和搜尋結果）。 然後，當您從快取中選取一或影像以進行失效時，URL會自動填入介面。
...
* **顯示編碼視訊** -依預設取消選取（關閉）。
   ...
   您也可以使用&#x200B;**[!UICONTROL File]** > **[!UICONTROL Reprocess]**&#x200B;直接從最適化視訊集建立更加編碼的「子」視訊。 Dynamic Media經典會自動尋找最適化視訊集的「父」主視訊，並將其當做原始視訊進行轉碼。 不過，在您儲存新的個別編碼的視訊後，搜尋或瀏覽時並不會看到這些視訊。然而這些視訊仍可從「詳細檢視」的「編碼的視訊」標籤予以存取。
...
* **預設影像對應HREF** -定義影像對應href欄使用的預設URL。此URL是您建立影像地圖時所看到的預設URL。
...
若要開啟「影像預設集」畫面，請在全域導覽列上按一下「設定&#x200B;**** > **[!UICONTROL 影像預設集]**」。
...
1. 在「添加（或編輯）預設」頁上，輸入預設的名稱。
1. 設定您想要的預設集選項。

   請參閱[影像預設集選項](application-setup.md#image_preset_options)。

1. 按一下&#x200B;**[!UICONTROL Save]**，或者，如果您從現有預設開始，請按一下&#x200B;**[!UICONTROL Save As]**。
1. 若要使用您自己的影像來預覽預設集，請按一下「瀏覽」，然後選取影像。 ****&#x200B;若要使用預設影像進行預覽，請按一下「重設&#x200B;**[!UICONTROL 」。]**
...
* **預設名稱** -輸入不含空白的描述性名稱。若要協助使用者識別此影像預設集，請在名稱中加入影像大小規格。

* **寬度和高度** -輸入影像傳送時的像素大小。

* **格式** -從菜單中選擇格式。選擇GIF、JPEG、PDF或TIFF格式會顯示更多選項：

   * GIF 色彩量化選項

      * **類型** -選擇「最適化」（預設值）、「網頁」或「Macintosh」。如果您選擇「含Alpha的GIF」，則無法使用Macintosh選項。
   ...
   * JPEG 選項
      ...
      * **品質** -控制JPEG壓縮等級。這個設定會同時影響檔案大小與影像品質。JPEG品質等級為1-100。
...
* **銳利化** -選取「啟用簡單銳利化」選項，在進行所有縮放後，將基本銳利化濾鏡套用至影像。銳利化可以幫助補償以不同大小顯示影像時所造成的模糊現象。

   如需更多有關銳利化、重新取樣模式和遮色片銳利化調整的資訊，請參閱[銳利化影像](sharpening-image.md#sharpening_an_image)。

* **重新取樣模式** -選擇重新取樣模式選項。下列選項會在縮減取樣影像時予以銳利化:

* **B-線性** -最快的重採樣方法；有些鋸齒不自然現象很明顯。

* **Bi-Cubic** -提高影像伺服器的CPU使用量，但產生更銳利的影像，並減少明顯的鋸齒不自然現象。

* **Sharp2**  —— 產生比雙三次方選項稍微銳利的結果，但影像伺服器的CPU成本更高。

* **Tri-Linear** -使用較高和較低的解析度（如果有的話）;僅當鋸齒是問題時建議使用。這個方法會減少高頻率資料，所以能夠減少 JPEG 大小。
...
* **閾值** -決定套用非銳利遮色片濾鏡時要忽略的對比範圍。換句話說，這個選項決定銳利化的像素必須與周圍的區域有多大的差異，才會被視為邊緣像素並予以銳利化。為了避免產生雜訊，請使用介於 .02 與 0.2 之間的值來實驗。預設值 6 會對影像中的所有像素進行銳利化。

* **色域** -決定影像使用的是建立影像的空間，通常是RGB（原始）或明度空間（強度）。

* **顏** 色選擇下列選項：

* **輸出顏色配置檔案** -選擇「使用預設值」或Dynamic Media經典版上可用的ICC顏色配置檔案之一。
...
1. 在「影像預設集」畫面中，選取表格中的預設集，然後執行以下一項操作:
   ...
   * 按一下&#x200B;**[!UICONTROL 編輯]** ，然後在編輯預設對話框中指定新選項。
   * 按一下&#x200B;**[!UICONTROL Delete]**&#x200B;從清單中刪除預設。
   * 取消選取預設名稱旁的&#x200B;**[!UICONTROL Active]**&#x200B;核取方塊，將它從MediaPortal使用者的整個Dynamic Media經典使用者介面中移除。
...
1. 在Dynamic Media經典影像的右上角附近，按一下「設定&#x200B;**** > **[!UICONTROL 應用程式設定]** > **[!UICONTROL 視訊預設集]** > **[!UICONTROL 最適化視訊預設集]**」。
...
>[!NOTE]
>
>除了「最適化視訊」（預設為啟用）之外，您無法在「上傳工作選項」對話方塊中看到所有其他最適化視訊或單一視訊編碼預設集。 Dynamic Media傳統管理員會決定哪些視訊編碼預設集會顯示在「上傳工作選項」對話方塊中。

* 從下列最適化視訊編碼或單一編碼預設集中選擇一項:

   * **16:9最適化視訊** -建立16:9外觀比例的視訊，以發佈至桌上型電腦、行動裝置(iPhone、iPad、Android™)和平板電腦(iPad、Android™)，並以最符合檢視器連線速度的解析度和位元速率最佳化。

   * **4:3最適化視訊** -建立4:3外觀比例的視訊，以發佈至桌上型電腦、行動裝置(iPhone、iPad、Android™)和平板電腦(iPad、Android™)，並以最符合檢視器連線速度的解析度和位元速率最佳化。

   * **最適化視訊** -單一編碼預設集，可搭配任何外觀比例建立視訊，以發佈至行動裝置、平板電腦和桌上型電腦。使用此預設集編碼而上載的來源視訊，會設定成固定的高度。不過，寬度會自動調整為保留視訊的外觀比例。
   ...
   * **最適化視訊編碼（16:9或4:3）** -建立16:9和4:3外觀比例視訊，以發佈至桌上型電腦、行動裝置(iPhone、iPad、Android™)和平板電腦(iPad、Android™)。所有功能都已最佳化，其解析度和位元速率最符合檢視器的連線速度。
   ...
   * **單一編碼預設集**
      ...
      * 您可以選取「最適化視訊編碼」和一或多個編碼預設集。 例如，您可以在一個上載工作中針對桌面和行動裝置編碼檔案。
...
* **每秒影格數(fps)** -每秒視訊的影格數，或靜止影像數。在美國和日本，大部分視訊以 29.97 fps 拍攝；在歐洲和亞洲 (不包括日本)，大部分視訊以 25 fps 拍攝。電影以 24 fps 拍攝。
...
| 4 | 4:3、384x288、行動(iPhone、iPad、Android™)、(600 Kbps) | 700 | _Mobile_384x288_600 | 600 | 384x288 | 與來源同 | 64 | 中等解析度、3G |
...
與來源 fps 相同。適用於iPhone、iPad和Android™行動裝置的視訊編碼預設集。
...
不同網頁有不同的需求。有時，您會想要網頁提供在個別瀏覽器視窗中開啟HTML5檢視器的連結。 在其他情況下，必須直接將HTML5檢視器內嵌在代管頁面上。 在後一種情況下，網頁可能具有靜態版面。 或者，它是「自適應」的，並在不同裝置或不同瀏覽器視窗大小上顯示不同的內容。 為滿足這些需求，隨附於Dynamic MediaClassic的HTML5檢視器支援靜態網頁和互動式設計網頁。
...
* **eCatalog Viewer**  —— 模擬閱讀列印目錄的體驗。您可以在頁面之間移動、放大和縮小頁面上的項目、使用影像地圖來查看頁面上項目的更多資訊，或搜尋目錄。 如果地圖區域擁有有效的 rollover_key 屬性，您也可以加入資訊面板以顯示詳細資訊和影像對應項目。若要加入資訊面板，請在「eCatalog 檢視器預設集」視窗中指定資訊伺服器 URL。
...
* 您可在下列位置找到支援此視訊格式的BlackBerry®裝置：[BlackBerry®](https://developer.blackberry.com/devzone/develop/supported_media/bb10_media_support.html)上支援的視訊格式
* 您也可以在下列位置找到支援此視訊格式的Windows®裝置：[Windows® Phone上支援的視訊格式](https://docs.microsoft.com/en-us/previous-versions/windows/apps/ff462087(v=vs.105)?redirectedfrom=MSDN)
...
* **添加預設** -按一下「添 **** 加」並在「添加查看器預設」對話框中選擇。

       請參閱[新增和編輯檢視器預設集](application-setup.md#adding_and_editing_viewer_presets)。
   
* **編輯預設** -選取預設，然後按一下「編 **[!UICONTROL 輯」]**。

       請參閱[新增和編輯檢視器預設集](application-setup.md#adding_and_editing_viewer_presets)。
   
* **刪除預設** -選擇預設，然後按一下「刪 **[!UICONTROL 除」]**。

* **匯出預設**  **** -選取HTML5檢視器預設集，然後按一下「匯出」以下載檢視器面板，讓您將它當做建立和新增其他檢視器預設集的基礎。

       請參閱[匯出HTML5檢視器預設集](application-setup.md#exporting_an_html5_viewer_preset)。
   
* **篩選檢視器預設集清單** -使用這些工具來篩選清單：

       *開啟**「活動中／非活動中」**下拉式清單，並選擇顯示活動中預設集、非活動中預設集或所有預設集的選項。
       *開啟**檢視器**下拉式清單，並選擇只檢視特定類型檢視器的選項。選擇**[!UICONTROL 所有檢視器]**可檢視所有檢視器。
   
* **排序預設集** -按一下列標題(**[!UICONTROL Active]**、Type **[!UICONTROL 、]** Preset **[!UICONTROL 或Platform]** ****)，將列上的清單排序。再按一下欄標題即可依遞減 (或遞增) 順序排列清單。

* **啟用和停用預設集** -選取預設集，然後按一下其「作用中」選項以啟用或停用預設集。

       請參閱[啟用或停用檢視器預設集](application-setup.md#activating_or_desacting_viewer_presets)。
   
>[!NOTE]
>
>按一下「檢視器預設集」頁面右側的&#x200B;**[!UICONTROL 預覽]**，查看您選取的檢視器預設集中資產的外觀。 若要查看不同的資產，請在「檢視器預設集」頁面中按一下「瀏覽&#x200B;****」，然後在「選取資產預覽」對話方塊中選取不同的資產。

### 增加和編輯檢視器預設集 {#adding-and-editing-viewer-presets}

除了在使用者介面中使用&#x200B;**[!UICONTROL Add]**&#x200B;新增檢視器預設集外，您也可以使用&#x200B;**[!UICONTROL Export]**&#x200B;新增檢視器預設集。 您只需匯出現有的HTML5檢視器預設集，然後將該預設集當做新預設集的基礎。
...
1. 在「檢視器預設集」頁面中，在「檢視器預設集」畫面上新增或編輯檢視器預設集。

   * **添加** -在工具欄上，按一下 **[!UICONTROL 添加]**。在「增加檢視器預設集」對話框中，選擇平台後，然後選擇多媒體資產類型。

          建立完檢視[!UICONTROL 器預設集後，按一下「另存新檔]」**。
      
   * **從現有的檢視器預設集開始新增** -在表格中，選取視訊檢視器預設集，然後按一下工具列上的編輯。

          在您重新設定視訊檢視器後，按一下「預設集名稱」文字欄位中的「另存新檔」**，以儲存預設集。
...
1. 設定其它想要的選項。
   ...
   >[!NOTE]
   >
   >按一下「與來源相同」，自動將視訊檢視器調整為編碼視訊本身的解析度大小。 ****&#x200B;如果選擇該選項，則不能輸入「舞台寬度」和「舞台高度」。相反，這些選項來自視訊本身。如果您選擇「與來源同」，請設定「邊距大小」選項以反映視訊播放區域外部的外觀尺寸。該邊距大小是視訊控制項的像素高度和寬度。您可以使用以下圖示來協助您決定要使用的邊距大小。*
...
您可以匯出現有的HTML5檢視器預設集，做為建立HTML5檢視器預設集的基礎。 匯出選項十分有用，因為您不需從頭建立檢視器， 但可以匯出與您要的外觀和行為接近的預設集，然後將其用作進行設計調整的起點。

所有預設的現成檢視器預設CSS檔案都位於「Dynamic Media經典」中，使用指向`Scene7SharedAssets`上資產的相對影像伺服路徑。 例如，以下是檢視器預設CSS檔案中影像資產的相對路徑：

`Scene7SharedAsset`: `.s7videoviewer .s7fullscreenbutton[state][selected] { background-image: url(/is/image/Scene7SharedAssets/FullScreenButton_dark_sprite?scl=1&fmt=png-alpha); }`

不過，如果您在自己的網站上代管檢視器CSS檔案，則必須在自己的環境中使用影像伺服器的明確路徑來解析這些相對影像路徑。 為了便於說明，如果要將上述相對路徑更新為顯式路徑，則其可能如下所示，其中`https://s7d1.scene7.com`是映像伺服器的直接路徑：`https://s7d1.scene7.com/is/image/Scene7SharedAssets/FullScreenButton_dark_sprite?scl=1&fmt=png-alpha`
...
1. 選擇「**[!UICONTROL Setup]** > **[!UICONTROL 檢視器預設集]**」。
...
1. 在「設定」窗口的左窗格中，展開&#x200B;**[!UICONTROL 應用程式設定]** > **[!UICONTROL 查看器]**
1. 按一下「**[!UICONTROL 預設查看器]**」。
1. 在「預設檢視器」視窗中，從每個資產類型下拉式清單中選取您要與資產預覽相關聯的檢視器。
1. 在「Default Viewers（預設查看器）」窗口的右下角，按一下「Save Settings（保存設定）」]**。**[!UICONTROL 
1. 在「設定」窗口的右下角，按一下&#x200B;**[!UICONTROL 關閉]**&#x200B;返回「資產」窗口。

## 中繼資料檢視 {#metadata-views}

*中繼資料*&#x200B;是有關資產的標準化資料。您可以使用中繼資料來簡化工作流程、組織資產和改善搜尋。Dynamic Media經典公司支援IPTC（國際新聞電信委員會）標準和XMP（可擴展元資料平台）標準。 在使用者檢視或在詳細資料檢視中輸入資產的中繼資料之前，他們可以開啟中繼資料檢視功能表。 在此處，他們可以選擇要檢視的中繼資料欄位集，或用來說明資產。
...
1. （可選）勾選「設定預設值&#x200B;**[!UICONTROL 」，讓使用者在「詳細資料檢視」中開啟「中繼資料」面板時，看到此檢視。]**
1. （可選）選擇「包括UDF」(**[!UICONTROL Include UDF]**)，在視圖中包括用戶定義的欄位。 使用者定義的欄位會顯示在詳細檢視的「中繼資料」面板上方。
1. 選擇要用於視圖的欄位（按一下&#x200B;**[!UICONTROL 選擇全部]**&#x200B;以選擇所有欄位）。
1. 按一下&#x200B;**[!UICONTROL 「儲存」]**。
...
1. 請執行下列任一動作:
   ...
   * 要編輯視圖，請選擇該視圖，然後按一下&#x200B;**[!UICONTROL 編輯]**。 然後，在「預覽」面板上選取或取消選取欄位名稱，並選取或取消選取「包含UDF」(**[!UICONTROL Include UDF]**)選項。
   * 要刪除視圖，請選擇該視圖，然後按一下&#x200B;**[!UICONTROL Delete]**。
   * 要使視圖成為預設視圖，請選擇該視圖，然後按一下&#x200B;**[!UICONTROL Make Default]**。 預設檢視為使用者在詳細檢視中開啟資產，並前往「中繼資料」面板時所看見的檢視。
...
1. 按一下「**[!UICONTROL 設定]** > **[!UICONTROL 應用程式設定]** > **[!UICONTROL 中繼資料]** > **[!UICONTROL 中繼資料預設集]**」。
1. 在「中繼資料預設集」面板中，執行下列任一項作業:

   * 要建立預設，請按一下&#x200B;**[!UICONTROL 添加]**。 在「中繼資料範本名稱」文字欄位中，輸入預設集的名稱，然後按一下「中繼資料檢視」**[!UICONTROL ，然後從下拉式清單中選擇檢視（請參閱[中繼資料檢視](application-setup.md#metadata_views)）。]**
   * 若要編輯現有預設，請從「中繼資料預設集」清單中選取預設集，然後按一下「編輯&#x200B;**[!UICONTROL 」。]**
...
1. 請執行下列任一動作:
   ...
   * 要刪除預設，請選擇該預設，然後按一下&#x200B;**[!UICONTROL Delete]**。
...
使用者定義的中繼資料欄位的有效用途之一，就是針對特定的上市發表會或銷售會，延遲資產的啟用時間。您可以根據&#x200B;*Date*&#x200B;類型定義「啟動」欄位。 然後，使用「詳細資料」檢視中的&#x200B;**[!UICONTROL 中繼資料]**&#x200B;面板或&#x200B;**[!UICONTROL 檔案]** > **[!UICONTROL 編輯資訊]**，您可以指定何時啟動資產。 Dynamic Media·經典會檢查資產的發佈狀態和發佈歷史記錄。 如果未在啟動時間內，則發佈狀態會顯示為「未發佈」。
...
1. 在「自訂欄位」對話框中，設定您要的選項。
   ...
   * **Type**  —— 選擇一個選項，用於定義用戶可以在元資料欄位中輸入的資訊類型：
   ...
   * **套用至** -（可選）如果希望元資料欄位僅應用於特定資產類型，請選擇資產類型。
      ...
      >[!NOTE]
      >
      >請謹慎選擇&#x200B;**[!UICONTROL 套用至]**&#x200B;選項，因為您無法在建立使用者定義的欄位後變更&#x200B;**[!UICONTROL 套用至]**&#x200B;選項。 Dynamic Media經典版可讓您編輯使用者定義欄位的名稱、類型和預設值，但不能編輯「套用至&#x200B;**[!UICONTROL 」設定。*]**
...
* **編輯欄位** -選取欄位，然後按一下「編 **[!UICONTROL 輯」]**。

* **刪除欄位** -選擇該欄位，然後按一下「刪 **[!UICONTROL 除」]**。

* **激活欄位** -按一下以選擇或取消選 **** 擇欄位名稱旁的「活動」選項。如果您是公司管理角色，則不會顯示此選項。 由於此選項與MediaPortal相關，因此您必須在個人設定中選取（開啟）「顯示MediaPortal功能」，以檢視啟動欄位。
...
1. 選擇「**[!UICONTROL 設定]** > **[!UICONTROL 應用程式設定]**」，然後選擇「優化檔案&#x200B;]**」。**[!UICONTROL 
...
公司管理員會先定義資產的命名慣例，以便在一組資產中進行群組。 然後，您可以建立批集預設集以參考這些影像。 每個預設集都是具備唯一名稱的內部獨立式指示集，定義如何使用與預設集配置中已定義的命名慣例相符的影像來建構集。
...
公司管理員會建立任何批次集預設集配置中使用的預設命名慣例。批集預設定義中選取的預設命名慣例，可能是您公司為所有網站批次產生集所需的所有功能。 必須建立批次集預設集，才能使用您所定義的預設命名慣例。公司定義的預設命名有例外情況時，您可以透過特定內容集所需的替代、自訂命名慣例來建立同等數量的批次集預設集。

使用批次集預設集功能不需要設定預設命名慣例。 但是，Adobe最佳做法建議您使用預設命名約定來定義要在一組中分組的命名約定的任意多個元素。 如此有助於簡化批次集的建立。
...
1. 選擇 **[!UICONTROL 「查看表單]** 」或「 **[!UICONTROL 查看代碼」]** ，以指定要查看的方式並輸入有關每個元素的資訊。
   ...
   您可以選取「檢視程式碼」核取方塊，檢視表格選擇旁邊建立的規則運算式值。 ****&#x200B;如果表單檢視因故對您有所限制，則您可以輸入或變更這些值，以幫助定義命名慣例的元素。如果無法在表單檢視中解析您的值，表單欄位會變為非作用中。
...
有兩個元素可供定義，分別為「符合」和「基本名稱」。這些欄位可讓您定義所有命名慣例元素，並識別慣例中用來命名其中所含之集的部分。公司的個別命名慣例可針對每個元素使用一或多行定義。 您可以針對唯一定義使用同等數量的行，並將其分組至不同的元素，例如「主影像」、「色彩元素」、「替代檢視」元素和「色票」元素。
...
使用「批次集預設集」即可建立、編輯和管理批次集預設集。您可以視需要建立同等數量的批次集預設集，以涵蓋您需要得所有資產擷取工作。批集預設定義有兩種形式：一個是您已設定的預設命名慣例，另一個是您即時建立的自訂命名慣例。
...
1. 在「預設集清單」面板中，按一下「新增&#x200B;****」，以啟用頁面右側「詳細資料」面板中的定義欄位。
...
1. 進行以下一項操作:
   ...
   * 如果您使用先前在&#x200B;**[!UICONTROL 應用程式設定]** > **[!UICONTROL 批次設定預設集]** > **[!UICONTROL 預設命名]**&#x200B;下設定的預設命名慣例，請展開&#x200B;**[!UICONTROL 資產命名慣例]**，然後在「檔案命名」下拉式清單中，按一下&#x200B;**[!UICONTROL default]**。
   * 若要在設定預設集時定義命名慣例，請展開「**[!UICONTROL 資產命名慣例]**」，然後在「檔案命名」下拉式清單中按一下「**[!UICONTROL 自訂]**」。

1. 對於「序列」順序，請定義在集合在Dynamic Media經典中分組後的影像順序。 根據預設，您的資產會按照字母順序排列。不過，您可以使用規則運算式以逗號分隔的清單來定義順序。
1. 針對「設定命名和建立慣例」，請為您在「資產命名慣例」中定義的基本名稱指定字尾或字首。也可定義在「Dynamic Media經典」資料夾結構內建立影像集的位置。

   如果您定義大量影像集，請將這些影像集與包含資產本身的檔案夾分開。 許多客戶會建立影像集檔案夾，並將應用程式重新導向，以將批次集所產生的集置放於此處。
...
回轉集的共用資產名稱部分的群組會新增至「符合」欄位（反白顯示）。 包含行和列的資產名稱的變數部分將分別添加到 行 和 列欄位中 。
...
1. 展開「**[!UICONTROL 列欄位置]**」，然後為影像資產在 2D 迴轉集陣列內的所在位置定義名稱格式。
   ...
   >[!NOTE]
   >
   >如果行和列規則運算式的組合無法確定資產在多維回轉集陣列中的位置，則不會將該資產添加到該集，並記錄錯誤。
