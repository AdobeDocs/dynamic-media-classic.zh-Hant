...
* **MP4**  -Dynamic Media經典推薦MP4作為首選視頻檔案格式。使用 MP4 檔案可進行以下動作:
...
* **預先編碼的視訊** -您直接將MP4檔案上傳至Dynamic Media經典。在此工作流程中，上載時不會將檔案編碼。檔案在準備傳送到桌面和行動裝置時會進行預先編碼。

* **主要來源影片** -上傳主要來源影片檔案，並在上傳時將這些檔案編碼為MP4檔案。在「瀏覽」面板中，編碼後的視訊會標記為「視訊」。Dynamic Media經典支援多種格式的視訊檔案編碼。
...
1. 在「上傳」頁面中，按一下「從案頭」]**頁籤。**[!UICONTROL 
1. 在「上傳」頁面上，在&#x200B;**[!UICONTROL 選擇要上傳的檔案]**&#x200B;面板中，按一下&#x200B;**[!UICONTROL 瀏覽]**，導覽至MP4視訊檔案，然後按一下&#x200B;**[!UICONTROL 開啟]**。
1. 在&#x200B;**[!UICONTROL 選擇資料夾目標]**&#x200B;面板中，為已上載檔案選擇一個資料夾。
1. 在「上傳」頁面上，請確定已勾選「上傳後發佈」。****
...
1. 在「全域導覽列」上，按一下「**[!UICONTROL 上載]**」。
1. 在「上傳」頁面中，按一下「從案頭」]**頁籤。**[!UICONTROL 
1. 在&#x200B;**[!UICONTROL 選擇要上傳的檔案]**&#x200B;面板中，按一下&#x200B;**[!UICONTROL 瀏覽]**，導航到主源視頻檔案，然後按一下&#x200B;**[!UICONTROL 開啟]**。
1. 在&#x200B;**[!UICONTROL 選擇資料夾目標]**&#x200B;面板中，為已上載檔案選擇一個資料夾。
1. 在頁面右下角，按一下「**[!UICONTROL 工作選項]**」。
1. 在「上載作業選項」對話框中，展開&#x200B;**[!UICONTROL 「視頻選項」]**，然後執行下列操作之一：

   * 最佳實務是選擇&#x200B;**[!UICONTROL 最適化視訊編碼]**。 請參閱[最適化視訊 (預設)](application-setup.md#adaptive-video-default)。
   * 選填。如果您想要使用個別的編碼設定，請展開&#x200B;**[!UICONTROL 單一編碼預設集]**，然後選取您想要的案頭、行動裝置和平板電腦的編碼選項。
...
1. 在「重新處理資產」對話框中，展開&#x200B;**[!UICONTROL 「EVideo選項」]**，然後執行下列操作之一：
...
* **視訊檔案格式** -類似ZIP檔案，視訊檔案格式會決定視訊檔案中檔案的包含方式。視訊檔案通常包含多個軌道:一個視訊軌道 (沒有音訊) 和一或多個音訊軌道 (沒有視訊)。這些軌道相互關聯並且同步。視訊檔案格式決定了這些不同資料軌道和中繼資料的組織方式。

* **視訊轉碼器** -視訊轉碼器描述視訊編碼的演算法。視訊播放器根據其轉碼器對視訊進行解碼，然後在畫面顯示一系列影像或影格。轉碼器將視訊檔案儲存為播放視訊所需的資訊量降至最低。只會儲存一個影格和下一個影格之間差異的相關資訊，而不儲存每個單獨影格的相關資訊。由於大部分視訊在一個影格之間幾乎沒有變化，所以轉碼器允許高壓縮率，因而檔案大小變小。
...
下表說明在對來源視訊檔案進行編碼時，建議的大小、外觀比例和最低位元速率：
...
1. 安裝後，以滑鼠右鍵按一下視訊檔案（僅限Windows®）並選取「MediaInfo」，或開啟「MediaInfo」，然後將視訊檔案拖曳至應用程式。 您會看到與視訊檔案相關的所有中繼資料，其中包括其寬度、高度和 fps。
...
當您選擇視訊編碼預設集時，請記得考量目標使用者的連線速度。 選擇資料速率為該速度 80% 的預設集。例如，如果目標終端使用者的連線速度為 1000 Kbps，最佳預設集是視訊資料速率為 800 Kbps 的預設集。
...
* 選擇資料速率，然後以最高解析度編碼，以您選擇的資料速率呈現最佳解析度。
...
「視訊預設集」頁面和「最適化視訊預設集」頁面由表格組成，其中列出每個視訊預設集的作用中狀態、預設集名稱、預期播放裝置、視訊大小和資料速率。
...
1. 在「Dynamic Media經典」中，在全域導覽列上，按一下「設定&#x200B;**** > **[!UICONTROL 應用程式設定]** > **[!UICONTROL 視訊預設集]** > **[!UICONTROL 最適化視訊預設集]**&#x200B;或&#x200B;**[!UICONTROL 單一編碼預設集]**」。

   「最適化視訊預設集」和「單一編碼預設集」的頁面包含一個表格，列出每個視訊預設集的「作用中」狀態、「預設集名稱」、「預設播放裝置」、視訊尺寸和資料速率。
...
已啟用的視訊預設集會顯示在「上載工作選項」對話框中。當使用者在上傳程式期間上傳視訊檔案時，會出現對話方塊。 他們可以從所有已啟用的編碼預設集中選擇。

1. 在「Dynamic Media經典」中，在全域導覽列上，按一下「設定&#x200B;**** > **[!UICONTROL 應用程式設定]** > **[!UICONTROL 視訊預設集]**」。
1. 進行以下一項操作:
...
您可以建立自己的自訂單一編碼視訊預設集，並將其增加到「視訊預設集」表格。您也可以變更Dynamic Media經典隨附的任何預先定義的單一編碼視訊預設集，前提是您必須以新名稱儲存編輯的預設集。
...
1. 在「視訊預設集」頁面中，執行下列任一項作業: 
   ...
   * 在「視訊預設集」工具列上，按一下「新增&#x200B;****」以新增視訊預設集。
   * 選取視訊預設集。按一下工具列中的「**[!UICONTROL 編輯]**」。

      您無法編輯Dynamic Media經典預先定義的預設集；您只能通過選擇&#x200B;**[!UICONTROL 另存為]**&#x200B;從現有預設建立預設。
...
1. 在「增加視訊預設集」頁面或「編輯視訊預設集」頁面上，設定您想要的視訊預設集選項。
   ...
   | 說明 | 描述視訊預設集。當您將指標移至「上傳工作選項」對話方塊中預設集的名稱上時，您輸入的內容會顯示為工具提示，使用者可在其中選擇轉碼選項。 |
   ...
   | 解析度大小 | 解析度大小由寬像素數和高像素數表示，決定大小。 以像素為單位輸入寬度和高度值，或拖曳滑桿來輸入這些值。解析度範圍列出了標準解析度大小。寬度和高度值將自動遵循所選的外觀比例。例如，如果您選取 4:3 作為外觀比例，並且為寬度輸入 400，則會自動為高度輸入 300。如果您對「外觀比例」設定選取「自動縮放」，「解析度大小」的「寬度」值會自動設為「自動」。按一下「預覽」，以開啟瀏覽器視窗並查看您的解析度選擇。 |
