---
title: 建立和啟用影像預設
description: 瞭解如何在Adobe Dynamic Media Classic建立和啟用影像預設。
...
# 建立和啟用影像預設{#creating-and-enabling-image-presets}

使用者使用 Media Portal 匯出影像資產時，可以在「匯出選取的資產」對話框中，選擇影像預設集。「影像預設集」是預先定義之設定的集合，而這些設定可在匯出影像時，變更影像外觀的大小、影像品質、格式、解析度及其他各方面。

Media Portal 管理員可以建立影像預設集，來控制影像匯出時重新格式化的方式。當用戶從Adobe Dynamic Media Classic導出影像時，「影像預設」會根據您公司的規格重新格式化影像。 使用者可匯出完全符合影像預設集規格的影像，而非自行重新格式化影像。
...
>[!NOTE]
>
>這些限制僅適用於導出的映像資產的導出，而不適用於主檔案的導出。
