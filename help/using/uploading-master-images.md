...
在將影像上傳至Adobe Dynamic Media Classic之前，請確定它們為最高品質的大小和格式。 Adobe Dynamic Media Classic建議上傳畫素數充足（長維度從1500到2000畫素）的高品質影像。 此大小調整可允許任何所需的Dynamic Imaging。
...
* **檔案格式**： Adobe Dynamic Media Classic支援所有標準影像檔案格式。 這些格式包括TIFF、BMP、JPEG、PSD、GIF和EPS。 建議使用不失真影像格式—TIFF 和 PNG。 如果您使用JPEG影像，請使用最高品質設定。

* **色域**： RGB是網頁影像簡報的色域。 上傳通常用於列印的CMYK影像時，會轉換為RGB。 建議上傳內嵌ICC （國際色彩聯盟）色彩設定檔的CMYK影像，以轉換至RGB。 另請參閱[ICC （國際色彩聯盟）設定檔](/help/using/icc-profiles.md)。
