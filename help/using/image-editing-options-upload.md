...
上傳AI、EPS和PSD檔案時，您可以在[!UICONTROL 上傳工作選項]對話方塊中執行這些動作：
...
若要手動裁切影像的側邊，請選取「裁切」選單，然後選擇「手動」。 接著輸入要從影像任一側或每一側裁切的像素數目。 裁切的影像量取決於影像檔案中的ppi （每英吋畫素）設定。 例如，假設影像顯示150 ppi。 然後在「上」、「右」、「下」和「左」文字方塊中輸入75。 此時，每一側都會裁切0.5英吋。
...
* **[!UICONTROL 自訂從]** > **[!UICONTROL 到]**：開啟功能表，讓您能夠選擇「轉換自」和「轉換至」色域。 您可以選擇標準的Adobe Photoshop色域，或您上傳至Adobe Dynamic Media Classic的色域。
...
此效果使用與Adobe Photoshop的「遮色片銳利化調整」濾鏡相同的選項。 雖然名稱可能會造成混淆，「不銳利化遮色片」是一種銳利化濾鏡。
...
| 臨界值 | 臨界值是在套用遮色片銳利化調整時忽略的對比範圍。 此效果很重要，因此使用此濾鏡時，影像不會引入「雜訊」。 值範圍是 0-255，表示灰階影像中亮度步階的數值。 0=黑色、128=50%灰色和255=白色。<br><br>例如，閾值為12會忽略膚色亮度的細微變化，以避免增加雜訊，但還是會增加邊緣對比度，以取得相異區域，例如睫毛與皮膚相遇的區域。<br><br>例如，如果您有某人的臉部像片，「不銳利化遮色片」會影響影像的反差部分。 例如，睫毛和皮膚會合，以建立明顯的對比區域，以及平滑的皮膚本身。 即使是最平滑的皮膚也會顯現亮度值的細微變化。 如果您不使用臨界值，則濾鏡會在皮膚像素中強調這些細微的變化。 因而產生雜訊和不想要的效果，同時提高睫毛的對比，增強銳利度。<br><br>為了避免此問題，系統引入臨界值，告訴濾鏡忽略不會大幅改變對比度的畫素，例如平滑外觀。 <br><br>在先前顯示的拉鍊圖形中，請注意拉鍊旁的紋理。 由於臨界值太低，無法抑制雜訊，因此會顯示影像雜訊。 |
| 單色 | 選取此選項可取消遮色片銳利化影像亮度（強度）。<br><br>取消選取此選項可分別取消遮色片銳利化每個色彩元件。 |
...
使用「移除背景」可在您上傳影像時自動移除影像的背景。 此技巧可用來反白顯示特定物件並將其與複雜背景隔離。

| 背景去底色選項 | 說明 |
| --- | --- |
| 背景去底色 | 選取以啟用或「開啟」「去底色背景」功能與選項。 |
| 邊角 | 必填。<br>用來定義要去底色的背景顏色的影像轉角。<br>您可以選擇<b>左上、左下、右上或右下</b>。 |
| 填色方法 | 必要. <br>從您設定的「轉角」位置控制畫素透明度。<br>您可以從下列填色方法中選擇：<br>· <b>Flood Fill</b>：將所有符合您指定且連線至角落畫素的畫素變成透明。<br>· <b>符合畫素</b>：將所有符合的畫素變成透明，無論其在影像上的位置為何。 |
| 容許度 | 選擇性。<br>根據您設定的「角落」位置，控制畫素色彩比對的允許變化量。<br>使用0.0的值以完全符合畫素顏色。 或者，使用1.0的值來允許最大的變數。 |
