...
* **依欄標題排序**：選取標題以依狀態依名字、姓氏或電子郵件的字母順序排序所有使用者。 或者，依使用者角色或有效/無效狀態排序。
...
* **選擇報表簡報型別**：在報表型別功能表上，選取 **[!UICONTROL 摘要]**， **[!UICONTROL 每日]**，或 **[!UICONTROL 每月]** 以摘要形式、按日期或按月組織資料。 並非所有報告都提供此選項。

* **指定時段**：選擇選項以定義報表的時段，然後選取 **[!UICONTROL 更新]** 定義時段後：

* **預先定義的時段**：在預先定義的報表功能表上，選擇一個選項。 例如，選擇「上月」可擷取前一個月的資料。

* **自訂時段**：在預先定義的報表功能表中，選取 **[!UICONTROL 自訂]**. 然後選擇日期 **[!UICONTROL 開始月份]** (或 **[!UICONTROL 開始日期]**)功能表，以及「月數」 （或「天數」）功能表上的日期。 對於「網域」和「視訊內容報告」，您可以選擇特定開始和結束日期，以擷取報告資訊。

* **排序資料（僅限資料檢視）**：排序欄中的資訊。 選取欄的標題。 再次選取「 」可依遞減順序排序。

* **重新排列欄（僅限資料檢視）**：若要將欄移至資料網格上的不同位置，請拖曳其標題。
...
* **時間**：在最近24小時內，首次回報錯誤到上次回報錯誤的時間範圍。
...
* **反向連結**：指定初始請求來自的網站URL。 反向連結可以是任何具有影像連結的網站。
