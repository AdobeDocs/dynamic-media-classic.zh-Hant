...
1. 選取&#x200B;**[!UICONTROL 儲存]**&#x200B;以完成使用者設定。
   ...
   新使用者在新增後會收到一封歡迎電子郵件。 此電子郵件會提供暫時密碼，並說明如何登入Adobe Dynamic Media Classic。

   如果使用者未收到歡迎電子郵件，請讓他們前往Adobe Dynamic Media Classic登入頁面(https://s7sps1.scene7.com)並選取&#x200B;**[!UICONTROL 忘記我的密碼]**。 已重設密碼並傳送新的電子郵件。 如果使用者沒有收到電子郵件，且電子郵件不在他們的垃圾郵件資料夾中，請聯絡技術支援。
...
* **依欄位名稱篩選**：選取&#x200B;**[!UICONTROL 啟用依欄位篩選]**。 接著選取&#x200B;**[!UICONTROL 依欄位名稱]**&#x200B;功能表，選擇欄以篩選清單，然後選取「篩選字元」功能表並選擇字母。 您選擇的字母會篩選其中一欄的清單。 若要檢視完整清單，請取消選取&#x200B;**`Enable Filter By Field`**&#x200B;選項。

* **篩選出無效的使用者**：取消選取&#x200B;**[!UICONTROL 包含無效]**。 搜尋結果只會顯示系統中的使用者。 已從您管理的系統和帳戶中刪除無效的使用者。

* **依欄標題排序**：選取標題，依狀態、名字、姓氏或電子郵件排序所有使用者。 或者，依使用者角色或有效/無效狀態排序。
...
### 設定資料顯示選項 {#viewing-data-in-different-ways}
...
* **選擇報表簡報型別**：在[報表型別]功能表中，選取&#x200B;**[!UICONTROL 摘要]**、**[!UICONTROL 每日]**&#x200B;或&#x200B;**[!UICONTROL 每月]**，依摘要、日期或月份組織資料。 並非所有報告都提供此選項。
...
1. (選擇性) 執行下列任一項作業:
   ...
   * 若要複製實際影像的連結，請選取&#x200B;**[!UICONTROL URL複製URL]**。 若要移至影像並調查錯誤，請在瀏覽器視窗中貼上此連結。
...
* **時間**：從第一次回報錯誤到上次回報錯誤的時間範圍（在過去24小時內）。
