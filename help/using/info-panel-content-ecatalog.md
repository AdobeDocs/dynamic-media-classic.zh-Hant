...
* 「資訊面板」的「資料摘要」面板可讓您指定CSV檔案。 此檔案包含您要顯示在「資訊面板」滑鼠指向效果文字中的文字。 您也可以排定更新資訊的時間。
...
* 「對應頁面」檢視提供預覽針對特定「影像對應」所顯示之XML的選項。

## 設定eCatalogs的回應範本 {#set-up-a-response-template-for-ecatalogs}

您可以選取三個預設回應範本之一，以在「資訊面板」中顯示文字。 這些預設的回應範本決定您的資訊在「資訊面板」中的顯示方式：欄和列數、字型大小、字型和其他設定。 您可以選取預設的回應範本，或建立您自己的回應範本。
...
1. 選取回應範本:
   ...
   * 若要建立您自己的回應範本，請選取&#x200B;**[!UICONTROL 自訂]**。 在「使用者範本」方框中鍵入範本 XML 定義。 您可以使用預設集範本作為自己的起點。

1. （選擇性）在「預設回應」方塊中，輸入您要在Adobe Dynamic Media Classic擷取「影像地圖」資訊時發生錯誤，顯示的文字。 例如，如果系統收到公司和eCatalog名稱，但沒有滑鼠指向效果識別碼，則會針對使用者顯示此訊息。
1. 在「回應 TTL」方框中輸入快取資料前要等候的時數:

   * 如果資料在白天頻繁更新，則設定較低的數字。
   * 如果資料相對穩定且不需要在白天頻繁更新，請設定較高的數字。 預設為十小時。
...
使用「對應頁面」畫面，您可以檢視eCatalog特定頁面上影像地圖的「資訊面板」文字。
