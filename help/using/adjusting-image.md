...
Adobe Dynamic Media Classic提供多種調整影像外觀的指令。 您可以翻轉、旋轉、模糊、改變色彩平衡和為影像著色。 當您嘗試使用這些指令時，您可以觀察它們在您正在使用的影像上的效果。
...
1. 請使用`Adjust Editor`視窗右側的命令，以便調整影像：
   ...
   * 使用&#x200B;**[!UICONTROL 旋轉器]**&#x200B;滑桿來旋轉影像。 您可以在&#x200B;**[!UICONTROL 旋轉器]**&#x200B;欄位中輸入值，以旋轉影像。 正值表示順時針旋轉；負值表示逆時針旋轉。
   * 使用「模糊」滑桿或相應的方框，即可模糊影像。 值愈高，影像的模糊度愈高。
   * 使用「對比」、「亮度」、「飽和度」、「色相」及「色彩平衡」選項，即可調整色彩與亮度。 這些效果會一直累積。 例如，對「洋紅/綠色」設定的變更會增加到對「色相」設定的變更上。
   * 使用`Colorize`選項為影像上色，同時保留陰影和高光。 對於「色彩化」選項的變更也會一直累積。 從[亮度]功能表選擇[無補償] ]**以停用自動亮度補償。**[!UICONTROL &#x200B;若要保留原始影像的對比範圍，請將對比值設定為0，或指定值大於0的對比範圍。 值 100 會使對比最大化。 一般值介於30到70之間。

1. 完成影像調整時，請執行下列其中一項動作:

   * 選取&#x200B;**[!UICONTROL 儲存]**。

   * 若要取代原始影像，請選取&#x200B;**[!UICONTROL 另存新檔]**。
   ...
   * 若要將影像儲存為主要影像的另一個檢視，您可以為其建立別名。 選取&#x200B;**[!UICONTROL 另存新檔]**。
