...
1. 在「回應範本」下拉式清單中，執行下列任一操作:
   ...
   * 若要建立您自己的回應範本，請選取&#x200B;**[!UICONTROL 自訂]**。 在「使用者範本」文字方框中，輸入範本 XML 定義。 您可以使用已在文字方塊中定義的預設範本，作為您自己回應的基礎。

1. （選擇性）在「預設回應」方塊中，輸入您要在Adobe Dynamic Media Classic擷取「影像地圖」資訊時發生錯誤，顯示的文字。 例如，如果系統收到公司名稱和「影像集」名稱，但缺少變換影像識別碼，則會對使用者顯示此訊息。
...
1. 在「S7Info上傳」對話方塊中，瀏覽至您要使用的檔案，然後選取&#x200B;**[!UICONTROL 上傳]**。
   ...
   支援的檔案格式為使用UTF-16編碼的定位字元分隔檔案，以及使用ASCII編碼的CSV檔案。 如果是 CSV 檔案，則非 ASCII 字元必須使用 HTML 編碼。
