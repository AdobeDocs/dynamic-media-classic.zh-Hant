...
如果管理員將權限給予 Media Portal 使用者，這些使用者就可以在匯出影像時予以重新格式化。 例如，可以變更大小、檔案格式及影像品質。 在匯出影像時自動重新格式化影像，可免除個別重新格式化影像的麻煩，進而提高效率。 此外，管理員可以建立預設集，即預先建立的影像格式設定選項。 在匯出影像時予以重新格式化以符合公司規格時，可以使用預設集。

如果您使用使用者定義的轉換來匯出影像資產，或匯出原始主要影像，則以下兩項限制適用：
...
1. 在「影像預設集」視窗中，選取下列任一選項:
   ...
   * **啟用使用者定義的轉換**：選取此選項時，使用者可以在[匯出選取的Assets]視窗中，從&#x200B;**[!UICONTROL 大小]**&#x200B;下拉式清單中選擇[其他]。 接著，使用者可選擇度量單位，並指定所需的寬度和高度。 當使用者匯出或下載這些檔案時，影像檔案便會重新格式化。
