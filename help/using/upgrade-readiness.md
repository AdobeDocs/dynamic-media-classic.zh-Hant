...
| **階段2：準備** | 驗證功能使用情況 | 確認[!DNL Dynamic Media Classic]中使用的功能在[!DNL Dynamic Media]中可用。 請參閱[功能比較](/help/using/upgrade-feature-comparison.md)頁面。 [!DNL Dynamic Media]中尚未提供的主要功能包括：<br>·視覺設定器（影像作者、影像轉譯）。<br>·影像範本（1:1範本）。<br>· eCatalogs。<br>如果正在使用上述功能，升級仍可進行，並假設這些功能可透過[!DNL Dynamic Media Classic]存取。 |
