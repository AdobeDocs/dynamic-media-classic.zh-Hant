...
1. 進行以下一項操作:
   ...
   * 如果您正在使用格線或清單檢視中的影像，請從「編輯」下拉式清單中選取&#x200B;**[!UICONTROL 影像地圖]**。 或者，在[詳細資料檢視]中開啟，然後選取影像上方的&#x200B;**[!UICONTROL 影像地圖]**。
   * 如果您正在使用格線檢視或清單檢視中的迴轉集，請選取&#x200B;**[!UICONTROL 編輯]**。 或者，在[詳細資料檢視]中開啟，然後選取[編輯]。**&#x200B;** 選取影像資產，然後選取&#x200B;**[!UICONTROL 影像地圖]**。
   * 如果您正在網格、清單或詳細資料檢視中使用eCatalog，請選取&#x200B;**[!UICONTROL 編輯]**。 選取&#x200B;**[!UICONTROL 對應頁面]**&#x200B;索引標籤。
...
* **變更位置**：將指標移近影像地圖的框線，但不要移過框線。 當您看到移動圖示時，請將地圖拖曳到新位置。

* **變更大小和形狀**：您如何變更影像地圖的形狀和大小，取決於您是使用矩形或多邊形影像地圖：

>[!TIP]
>
>若要變更檢視並更清楚地檢視您的「影像地圖」，請拖曳畫面底部的「大小」滑桿。

* **矩形影像地圖**：將指標移到影像地圖的邊或角上。 當您看到雙向箭頭圖示時，請開始拖曳。 若要變更大小但維持外觀比例（形狀），請在拖曳時按住Shift鍵。
...
您可以在地圖摘要畫面中匯入影像、迴轉集或eCatalog的資料，而不需在每個頁面上輸入「影像地圖」資料。 匯入影像地圖資料時，可以採用 Tab 字元分隔檔案或 XML DTD 的格式。 檔案中的欄位必須遵循地圖摘要畫面顯示的順序：名稱、目錄標籤、地圖、URL、滑鼠指向效果文字、其他動作和搜尋字串。 匯入「影像地圖」資料可避免在建立每個「影像地圖」時需在「影像地圖」清單中輸入資料。
...
1. （選擇性）選取&#x200B;**[!UICONTROL 產生]**。 這會根據您要建立的檔案型別預覽產生的資料。 選取&#x200B;**[!UICONTROL 關閉]**&#x200B;以返回「上載中繼資料」對話方塊。
...
您可以將影像地圖從某個影像或 eCatalog 頁面複製到另一個影像或 eCatalog 頁面。 使用&#x200B;**[!UICONTROL 複製影像地圖]**&#x200B;以簡化建立程式。 若要在共用版面或對應結構的影像或頁面中建立「影像對應」，您也可以複製它們。

例如，在eCatalog中複製影像地圖是在同一eCatalog的不同語言版本之間複製所有影像地圖的便利方式。 為達到最佳效果，如果您在相同頁數的eCatalog與相同影像之間複製，復製作業將最成功。 如果您要複製的eCatalog已經包含「影像地圖」，則製作複製時會刪除這些「影像地圖」。

**複製影像地圖：**

1. 前往「影像地圖」編輯器頁面 (針對影像或迴轉集中的影像) 或是 eCatalog 編輯畫面的「地圖頁面」標籤。
1. 選取&#x200B;**[!UICONTROL 將地圖複製到]**。
1. 請根據您是從影像複製影像地圖，還是從eCatalog複製影像地圖，執行下列任一項作業：
...
若要簡化輸入影像地圖URL的程式，您可以定義URL範本（也稱為Href範本）。 如果大多數的影像地圖 URL 共用一個通用的固定格式，則可定義 URL 範本。 將固定的 URL 部分輸入 URL 範本之後，每次建立影像地圖時，就不需要輸入這個部分的 URL。 您的URL範本也可以包含JavaScript命令、路徑名稱和引數。 根據預設，URL範本包含名為`loadProduct`的專屬Adobe Dynamic Media Classic JavaScript處理常式，可在新視窗中開啟影像。

>[!NOTE]
>
>將JavaScript程式碼新增至影像地圖的HREF屬性時，系統會在使用者的電腦上處理該程式碼。 因此，請確定JavaScript程式碼是安全的。
...
您可以在URL範本中的影像地圖之間放置所有不會變更的值。 在「影像地圖」清單中，只新增變更至URL欄的值。 例如:
...
* `loadProductCW`：顯示目前視窗內URL欄位中指定的URL目標。 這個處理程式主要用於已整合成網站某一頁面的 eCatalog。

* `loadProductPW`：顯示父視窗（開啟目前視窗的頁面）之URL欄中所指定的URL目標。 目前視窗會維持開啟，但父級視窗會變更以顯示 URL 目標。
...
* **範本選項**：如果您不希望個別影像地圖使用URL範本，請取消選取「影像地圖」清單中的「範本」選項。
...
* **`onfocus`**：影像地圖透過游標、Tab鍵或按存取鍵接收焦點時觸發事件。 例如，您可以在影像地圖收到焦點時啟動網頁，並在影像地圖失去焦點時關閉網頁。

* **`onblur`**：影像地圖透過游標或Tab鍵失去焦點時會觸發事件。
...
1. 在Adobe InDesign中，移至&#x200B;**[!UICONTROL 視窗]** > **[!UICONTROL 互動式]** > **[!UICONTROL 超連結]**。
