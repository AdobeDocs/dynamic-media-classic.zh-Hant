...
1. 您可以讓檢視者在選取「影像地圖」時開啟新的網頁。 在「影像地圖」清單面板的「URL」欄中，輸入網頁的URL。
   ...
   若要簡化輸入URL （HREF範本）的程式，請選取&#x200B;**[!UICONTROL 編輯]**&#x200B;並輸入範本。
...
您可以使用eCatalog的Rich Media選項，將MP4格式的視訊或迴轉集新增至eCatalog的影像地圖。 當使用者在eCatalog中選取「影像地圖」區域時，會顯示相關的「迴轉集」或視訊。 如果您希望客戶在使用期間檢視料號，或是從不同角度檢視料號，此功能特別有用。
...
* **調整位置**：選取「平移」工具，將指標移到地圖邊界附近，不要放在地圖上。 當指標顯示方向箭頭圖示時，請將整個「影像地圖」拖曳到新位置。
...
>[!NOTE]
>
>當您將「影像地圖」複製到eCatalog中的不同頁面時，也可以將一個eCatalog中的所有影像地圖複製到另一個。 請參閱[在eCatalogs](creating-ecatalog-image-maps.md#copying_image_maps_between_ecatalogs)之間複製影像地圖。

## 檢閱及匯入影像地圖資料 {#reviewing-and-importing-image-map-data}

「地圖摘要」畫面提供有關 eCatalog 的中繼資料。 在「地圖摘要」畫面上開始作業時，您也可以為 eCatalog 批次匯入影像地圖資料。 以這種方式匯入影像地圖資料，可簡化輸入影像地圖URL和滑鼠指向效果文字的程式。
...
1. 如果出現滑鼠指向效果錯誤，請選取&#x200B;**[!UICONTROL 滑鼠指向效果索引鍵錯誤]**&#x200B;欄中的錯誤，以檢視試算表中必須更新的專案來更正錯誤。 您可以從這個訊息選取並複製文字，然後將其貼上試算表。
1. 選取「**[!UICONTROL 預覽]**」，以便在eCatalog檢視器中檢查頁面。 選取X以關閉[摘要]畫面並返回[地圖頁面]畫面，或選取&#x200B;**[!UICONTROL 關閉]**&#x200B;以返回[瀏覽]。

### 匯入影像地圖資料 {#import-image-map-data}

您可以將整個eCatalog的資料匯入「地圖摘要」畫面，而不需在每個頁面上輸入「影像地圖」資料。 匯入影像地圖資料時，可以採用 Tab 字元分隔檔案或 XML DTD 的格式。 檔案中的欄位必須遵循地圖摘要畫面顯示的順序：名稱、目錄標籤、地圖、URL、滑鼠指向效果文字、其他動作和搜尋字串。 匯入「影像地圖」資料後，您就不需要在建立每個「影像地圖」時輸入「影像地圖」清單中的資料。
...
1. 在「工作名稱」(Job Name)欄位中，輸入檔案的名稱（請確定保留其副檔名）。
1. 選取&#x200B;**[!UICONTROL 上傳]**。

## 在eCatalog之間複製影像地圖 {#copying-image-maps-between-ecatalogs}

您可以將一個 eCatalog 中的所有影像地圖複製到其它 eCatalog。 以這種方式複製影像地圖是在相同eCatalog的當地語系化版本之間複製影像地圖的方法。 為了成功複製，Adobe Dynamic Media Classic建議您在具有相同頁數和相同影像大小的eCatalog之間複製。
...
如果您將影像地圖複製到的目標eCatalog具有不同數量的頁面或不同大小的影像，Adobe Dynamic Media Classic會顯示警告訊息。 選取&#x200B;**[!UICONTROL 繼續]**&#x200B;以複製影像地圖，無論是否有警告。
