...
* **[!UICONTROL 已發佈的伺服器名稱]**：此伺服器是所有系統產生、特定於您帳戶的URL呼叫中所使用的即時CDN （內容傳遞網路）伺服器。 只有在Adobe Dynamic Media Classic支援代表指示您變更此伺服器名稱時，才會進行變更。

* **[!UICONTROL 原始伺服器名稱]**：此伺服器僅用於品質保證測試。 只有在Adobe Dynamic Media Classic支援代表指示您變更此伺服器名稱時，才會進行變更。
...
* **[!UICONTROL 顯示產生的內容]**：在資料夾中，它顯示從資產產生的內容。 例如，PDF檔案上傳時點陣化時，Adobe Dynamic Media Classic會為原始PDF中的每個頁面建立一個影像。 如果選取了「顯示產生的內容」 ，則上傳原始PDF時每個產生的影像都會顯示。 它會與PDF一起出現在PDF上傳到的資料夾中。
...
* **[!UICONTROL 保留發佈]**：指定上傳至Adobe Dynamic Media Classic的取代影像是否保留其取代影像的「準備發佈」設定。 或者，在上傳時指定設定。
...
* **[!UICONTROL 預設影像對應範本]**：為影像對應中的HREF範本定義預設JavaScript。 若要在每次選取影像地圖時執行自訂程式碼，請在此處設定。

### 其他設定（至應用程式）

* **[!UICONTROL 垃圾桶清理警告]**：垃圾桶資料夾中的Assets會在七天內自動刪除。 選取「在自動刪除垃圾桶資料夾中的專案之前傳送電子郵件」。 此動作會傳送通知給公司管理員。 當垃圾桶資料夾中的資產排程在四天內永久刪除時，就會發生這種情況。 請參閱[管理垃圾桶](/help/using/trash-folder.md)。
...
如果上傳的影像以某種方式變更（影像已變更），但影像的參照保持不變，請使用此選項。 上傳和擷取® PDF時，覆寫也會很有幫助。 您可以微調Adobe Dynamic Media Classic *`rips`*&#x200B;影像的方式。 您也可以在「上傳」對話方塊中調整ICC色彩設定檔選項，並使用覆寫功能重新上傳。
...
「新增預設集」和「編輯預設集」畫面提供以下建立和編輯影像預設集的選項：
...
* **[!UICONTROL 格式]**：從功能表選取格式。 選擇GIF、JPEG、PDF或TIFF格式會提供更多選項：
  ...
  * GIF 色彩量化選項
    ...
    * **[!UICONTROL 型別]**：選取最適化（預設）、Web或Mac。 如果您選取&#x200B;**[!UICONTROL 具有Alpha的GIF]**，則無法使用Mac選項。
  ...
  * JPEG 選項
    ...
    * **[!UICONTROL 品質]**：控制JPEG壓縮等級。 這個設定會同時影響檔案大小與影像品質。 JPEG品質刻度是1至100。

    * **[!UICONTROL 啟用JPG色度縮減取樣]**：由於眼睛對高頻色彩資訊的敏感性低於高頻明度，因此JPEG影像會將影像資訊分為明度和色彩元件。 壓縮 JPEG 影像時，明度組件會保留完整解析度，而色彩組件則會透過平均像素群組來縮減取樣。 縮減取樣將資料量減少一半或三分之一，幾乎不影響感知品質。 縮減取樣不適用於灰階影像。 這個技巧可減少適用於高對比度影像的壓縮量 (例如含有覆蓋文字的影像)。
...
* **[!UICONTROL 雙立方式]**：增加影像伺服器上的CPU使用量，但會產生較清晰的影像，且鋸齒狀不自然感比較不明顯。

* **[!UICONTROL `Sharp 2`]**：產生的結果可能會比「雙立方式」選項稍微銳利，但耗用的影像伺服器上的CPU成本更高。

* **[!UICONTROL 三線性式]**：同時使用較高和較低解析度（如果有的話）；建議只在出現鋸齒問題時才使用。 這個方法會減少高頻率資料，所以能夠減少 JPEG 大小。

* **[!UICONTROL 不銳利化遮色片]**：選擇下列選項以微調銳利化：

* **[!UICONTROL 數量]**：控制套用至邊緣畫素的對比數量。 預設值為1.0。 若是高解析度的影像，最高可增加到5.0。 將「數量」視為濾鏡強度的量度。

* **[!UICONTROL 半徑]**：決定邊緣畫素周圍影響銳利化的畫素數量。 對於高解析度的影像，請輸入 1 到 2 的數字。 低數值只會銳利化邊緣的像素；高數值會銳利化較寬的像素範圍。 正確的值取決於影像大小。

* **[!UICONTROL 臨界值]**：若要在套用遮色片銳利化調整濾鏡時忽略對比範圍，請使用此設定。 也就是說，它可以協助解決銳化畫素與周圍區域必須有多大的差異，才會被視為邊緣畫素並加以銳化。 為避免產生雜訊，請使用介於`.02`到`0.2`之間的值來實驗。 預設值6會銳利化影像中的所有畫素。
...
* **[!UICONTROL 內嵌設定檔]**：選取此選項，以便當您在® Photoshop®中開啟此影像時，它會使用此設定檔。
...
依預設，僅會啟用「最適化視訊」編碼預設集（啟用或「開啟」）。 如有需要，您可以將其停用。 非作用中的最適化視訊預設集，並不會以可選取的選項顯示在「上載工作選項」對話框的「eVideo」區段中。
...
1. 若要從「上傳工作選項」對話方塊的「eVideo選項」清單中移除預設集，請取消選取「最適化視訊預設集」頁面上預設集名稱旁的核取方塊。
1. 選取&#x200B;**[!UICONTROL 關閉]**。
...
>[!NOTE]
>
>除了Adobe Dynamic Media Classic預設啟用的「最適化視訊」之外，Adobe Dynamic Media Classic管理員會決定哪些單一編碼視訊編碼預設集會顯示在「上傳工作選項」對話方塊中。 Adobe Dynamic Media Classic管理員會決定哪些視訊編碼預設集會顯示在「上傳工作選項」對話方塊中。
...
* 從下列最適化視訊編碼或單一編碼預設集中選取：
  ...
  * **[!UICONTROL 最適化視訊]**：單一編碼預設集，可與任何外觀比例搭配使用，建立視訊以傳送至行動裝置、平板電腦和案頭。 以此預設集編碼的已上傳來源視訊有固定高度。 不過，寬度會自動調整比例，以保留視訊的外觀比例。
...
*檢視器預設集*&#x200B;是決定使用者如何在電腦螢幕和行動裝置上檢視多媒體資產的設定集合。 身為一名管理員，您可以建立檢視器預設集。 設定可用於檢視器設定選項的陣列。 例如，您可以變更檢視器的顯示大小、縮放行為及外觀。

如需參考最佳作法，請使用Adobe Dynamic Media Classic HTML5視訊檢視器。 HTML5 視訊檢視器使用的預設集是健全的視訊播放器。
...
不同的網頁有不同的需求。 有時候，您會想要有網頁提供連結，在個別瀏覽器視窗中開啟HTML5檢視器。 在其他情況下，需要直接將HTML5 Viewer內嵌在託管頁面上。 在後一種情況下，網頁會具有靜態配置。 或是「回應式」功能，且在不同裝置或不同瀏覽器視窗大小中顯示的方式有所不同。 為因應這些需求，Adobe Dynamic Media Classic隨附的HTML5檢視器同時支援靜態網頁和回應式設計的網頁。
...
* **[!UICONTROL 混合媒體集檢視器]**：在一個檢視器中顯示不同型別的媒體。 您可以加入色票集、迴轉集、影像和視訊。 若要包含不同型別的內容，請設定標籤，例如影像集的標籤和影片的標籤。 從混合媒體集播放的視訊會使用標準視訊檢視器，該檢視器具有時間軸和視訊控制項，例如「停止」、「暫停」、「倒帶」和「播放」。 當您設定混合媒體集檢視器預設集時，請指定您要用於混合媒體集中不同資產型別的檢視器。 您也可以使用格線檢視器或轉盤檢視器來檢視混合媒體集。
...
* **[!UICONTROL 縮放檢視器：彈出]**：在原始影像旁邊顯示縮放區域的第二個影像。 沒有控制項；使用者將選取範圍移動到他們想要檢視的區域上。
...
例如，如果您的主影像大小為 350 x 350 像素且「縮放係數」為 3，則產生的彈出影像為 1050 x 1050 像素。 如果您的主影像大小為 300 x 300 像素且「縮放係數」為 4，則彈出影像為 1200 x 1200 像素。 視JPEG品質設定（建議的設定介於80到90之間）而定，您可以縮小檔案大小。 建議的縮放係數為 2.5 至 4，視主影像大小而定。
...
|  | 檢視器技術 | 桌上型電腦 | Apple iPhone | Apple iPad | ™智慧型手機 | ™平板電腦 |
|--- |--- |--- |--- |--- |--- |--- |
| 縮放檢視器 |  |  |  |  |  |  |
...
|--- |--- |--- |--- |--- |--- |--- |
| eCatalog檢視器 |  |  |  |  |  |  |
| Universal_HTML5_eCatalog_Adv（包含對社群媒體和目錄搜尋的支援）。 | HTML5 | X | X | X | X | X |
...
* 您可以在下列位置找到支援此視訊格式的BlackBerry®裝置： [BlackBerry®](https://www.blackberry.com/en/secure-communications/support/developers)支援的視訊格式
...
|  | 檢視器技術 | 桌上型電腦 | Apple iPhone | Apple iPad | ™智慧型手機 | ™平板電腦 | BlackBerry®智慧型手機 | Windows® Phone |
|--- |--- |--- |--- |--- |--- |--- |--- |--- |
| Universal_HTML5_Video（包含隱藏式字幕支援。） 請參閱[最佳實務：使用Universal HTML5視訊檢視器。](best-practice-using-html5-video.md#best_practice_using_the_html5_video_viewer) | HTML5 | X | X | X | X | X | X | X |
...
除了在使用者介面中使用&#x200B;**[!UICONTROL 新增]**&#x200B;來新增檢視器預設集之外，您也可以使用&#x200B;**[!UICONTROL 匯出]**&#x200B;來新增檢視器預設集。 您匯出現有的HTML5檢視器預設集，然後使用該預設集作為新預設集的基礎。
...
1. 在「檢視器預設集」頁面中，新增或編輯「檢視器預設集」畫面上的「檢視器預設集」 。
   ...
   * **新增**：在工具列上，選取&#x200B;**[!UICONTROL 新增]**。 在「新增檢視器預設集」對話方塊中，選取平台，然後選取多媒體資產型別。
     ...
     完成檢視器預設集的建立後，選取&#x200B;**[!UICONTROL 另存新檔]**。
...
若要建立HTML5檢視器預設集，請匯出現有的HTML5檢視器預設集以用作基礎。 此匯出選項相當實用，因為您不需要從頭開始建立檢視器。 相反地，您會匯出與您想要的相類似的預設集，然後您就可以以此為基礎進行設計調整。

Adobe Dynamic Media Classic中所有預設的標準檢視器預設集CSS檔案都會使用指向`Scene7SharedAssets`上資產的相對影像伺服路徑。 例如，以下為上檢視器預設集CSS檔案中影像資產的相對路徑

`Scene7SharedAsset`: `.s7videoviewer .s7fullscreenbutton[state][selected] { background-image: url(/is/image/Scene7SharedAssets/FullScreenButton_dark_sprite?scl=1&fmt=png-alpha); }`

不過，如果您將Viewer CSS檔案託管在您自己的網站上，則必須在您自己的環境中使用影像伺服器的明確路徑來解析這些相對影像路徑。 例如，如果您將上方的相對路徑更新為明確路徑，如下所示，其中`https://s7d1.scene7.com`是影像伺服器的直接路徑： `https://s7d1.scene7.com/is/image/Scene7SharedAssets/FullScreenButton_dark_sprite?scl=1&fmt=png-alpha`
...
中繼資料預設集可讓管理員控制並規範指派給資產的中繼資料。 在「詳細資料檢視」中，使用者可在為此目的提供的欄位中輸入有關資產的中繼資料。 例如，使用者可以輸入擁有者名稱、版權描述和地址。 為確保使用者正確輸入此資訊，您可以建立中繼資料預設集。 在「詳細資料檢視」中選擇中繼資料預設集，會以預先定義的值填入中繼資料欄位。 例如，擁有者名稱、版權描述和地址皆會自動輸入。
...
Media Portal管理員或公司管理員可以建立自訂、使用者定義的中繼資料欄位。 自訂欄位可協助您在Adobe Dynamic Media Classic中組織資產。 您可以視需要將欄位標示為「使用中」。 啟動後，這些自訂中繼資料欄位名稱會顯示在「詳細資料檢視中繼資料」面板中。 使用者可以在使用者自訂的中繼資料欄位中輸入資訊，以描述資產。 使用者也可以將使用者定義的中繼資料欄位設定作為搜尋條件。
...
>[!NOTE]
>
>若要讓使用者定義的欄位顯示在「詳細資料檢視中繼資料」面板中，請在中繼資料檢視中加入使用者定義的欄位。 在[中繼資料檢視]畫面上，選取&#x200B;**[!UICONTROL `Include UDF (user-defined fields)`]**&#x200B;選項。 如需詳細資訊，請參閱[中繼資料檢視](application-setup.md#metadata_views)。
...
1. 在「自訂欄位」對話框中，設定您要的選項。
   ...
   * **[!UICONTROL 預設值]**：選擇性。 輸入使用者在欄位中輸入頻率最高的值。 您輸入的值會成為您建立之欄位的預設值。

   * **[!UICONTROL 套用至]**：選擇性。 如果您希望中繼資料欄位僅套用至特定資產型別，請選取資產型別。
...
只有Media Portal管理員或公司管理員可以管理使用者定義的欄位。
...
公司管理員首先會為想要分組到資產集的資產定義命名慣例。 若要參照這些影像，請建立「批次集預設集」。 每個預設集都是一組名稱唯一、內含的指示。 它會定義如何使用符合預設方式中定義之命名慣例的影像來建構集合。
...
公司管理員會建立用於任何「批次集預設集」配方的預設命名慣例。 在「批次集預設集」定義中選取的預設命名慣例，正是貴公司為所有網站批次產生集所需的全部。 建立`Batch Set Preset`以使用您定義的預設命名慣例。 您可以針對特定內容集建立所需數量的批次集預設集，並搭配替代自訂命名慣例，以因應公司定義預設命名例外情況。
...
1. 選取「**[!UICONTROL 檢視表單]**」或「**[!UICONTROL 檢視器碼]**」即可指定您要如何檢視和輸入個別元素的相關資訊。
   ...
   您可以選取&#x200B;**[!UICONTROL 檢視程式碼]**&#x200B;核取方塊，以檢視表單選取專案旁邊的規則運算式值建置。 如果「表單檢視」因任何原因限制您，您就可以輸入或變更這些值，以協助定義命名慣例的元素。 如果無法在表單檢視中剖析您的值，則表單欄位會變成非使用中。
...
有兩個元素可供定義，分別為「符合」和「基本名稱」。 這些欄位是您為命名慣例定義的所有元素。 它們有助於識別用來命名集合的慣例部分。 此集包含這些變數。 公司的個別命名慣例會針對這些元素中的每一個使用一或多行定義。 您可以針對唯一的定義使用儘可能多的線條，並將它們分組為不同的元素，例如影像、顏色和檢視元素。
...
使用批次集預設集來建立、編輯和管理您的批次集預設集。 若要涵蓋所有必要的資產擷取工作，請視需要建立儘量多的預設集。 批次集預設集定義有兩種形式：一種是您設定的預設命名慣例，另一種是您立即建立的自訂命名慣例。

您可以使用表單欄位方法來定義批次集預設集，或使用代碼方法來讓您使用規則運算式。 如同在&#x200B;**[!UICONTROL 預設命名]**&#x200B;中一樣，您可以在表單檢視中定義的同時選取&#x200B;**[!UICONTROL 程式碼檢視]**，並使用規則運算式來建置您的定義。 您也可以取消核取任一檢視，僅使用其中一種。
...
您可以使用批次集型別&#x200B;**多軸迴轉集**&#x200B;來建立「方式」，自動產生2D迴轉集。 影像群組會使用Row和Column規則運算式，對齊多維度陣列中的影像資產。
