...
* **依群組篩選** -選取「依群組」功能表，然後選擇選項，將清單縮小至群組中的使用者。

* **按用戶角色篩選** -選擇按用戶角色菜單，然後選擇一個選項，將清單限定給不同類型的用戶或管理員。

* **依欄位名稱篩選** -選取「啟用依欄位篩選」選項。然後選取「依欄位名稱」選單、選擇篩選清單的欄，然後選取「篩選字元」選單並選擇一個字母。清單會依您選擇的字母列出其中一個欄。取消選取「啟用依欄位篩選」選項，即可查看完整清單。

* **過濾掉無效用戶** -取消選擇「包括無效」選項。搜尋結果只會顯示系統中的使用者。已從系統和您所管理的帳戶中刪除無效的使用者。
...
* **在圖表或資料網格中查看資料** -按一下「圖表視圖」選項查看圖表中的資料；按一下「資料檢視」選項，以檢視資料格線中的資料。

* **選擇報表簡報類型** -在「報表類型」功能表中，選擇「摘要」、「每日」或「每月」，以匯總形式、依日或依月組織資料。並非所有報告都提供此選項。

* **指定時段** -選擇選項以定義報表的時段，然後在定義時段後按一下「更新」:

* **預先定義的時段** -在「預先定義的報表」功能表上，選擇一個選項。例如，選擇「上月」可擷取前一個月的資料。

* **自訂時段** -在「預先定義報表」功能表上，選擇自訂。然後在「開始月份」(或「開始日期」) 選單上選擇日期，並在「月數」(或「天數」) 選單上選擇日期。對於「網域」和「視訊內容報告」，您可以選擇特定開始和結束日期，以擷取報告資訊。
...
* **匯出報表資料** -在「資料」檢視中，視需要排序及排列資料。然後開啟「匯出」選單，並選擇格式:「Tab 字元分隔」、「逗號分隔」或「HTML 格式」。資料會依您選擇的格式複製到剪貼簿。您現在可以將資料貼上試算表或應用程式。
...
1. (選擇性) 執行下列任一項作業:
   ...
   * 按一下「URL複製URL ]**」，將連結複製至實際影像。**[!UICONTROL &#x200B;您可以在瀏覽器視窗中貼上此連結，以前往影像並調查錯誤。
   * 按一下「**[!UICONTROL 反向連結複製URL]**」，將連結複製至反向連結網頁。
