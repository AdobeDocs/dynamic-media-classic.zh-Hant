...
1. 選擇要指派給該使用者的「角色」選項。
   ...
   請參 [閱Dynamic Media Classic使用者角色和權限](administration-setup.md#user_administration)。
...
**依使用者角色篩選** ：選取依使用者角色選單，然後選擇選項，將清單縮小為不同類型的使用者或管理員。

**依欄位名稱篩選** ：選取「啟用依欄位篩選」選項。 然後選取「依欄位名稱」選單、選擇篩選清單的欄，然後選取「篩選字元」選單並選擇一個字母。清單會依您選擇的字母列出其中一個欄。取消選取「啟用依欄位篩選」選項，即可查看完整清單。

**過濾掉無效用戶** 取消選擇包含無效選項。 搜尋結果只會顯示系統中的使用者。已從系統和您所管理的帳戶中刪除無效的使用者。

**按欄標題排序** ：按一下標題，依使用者狀態、依名字、姓氏或電子郵件的字母順序、依使用者角色或有效／無效狀態來排序所有使用者。
...
**遮色片請求** -將請求遮色片傳回灰階遮色片的影像。

**檢視器載入的檢視器拼貼** 「請求影像」請求。

**Vnt物件要求** ：影像轉換要求，會傳回在請求的暈映中具有指定物件的影像。

**Vnt Info Request** Image rendering requests that return information abount the requested vignettes.
...
**指定時段** ：選擇選項以定義報表的時段，然後在定義時段後按一下更新：
...
**列印報表** 「按一下列印」，在「列印」對話方塊中選擇您要的選項，然後按一下「確定」。
