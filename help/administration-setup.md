...
所有Dynamic Media Classic使用者都會獲派角色，以決定其Scene7 Publishing System中功能的權限和存取權限。 管理員會決定不同角色和其對於所指派公司的責任。
...
**SPS管理員** —可檢視和管理Scene7 Publishing System中的所有功能，以及設定公司並新增管理員和使用者。

**公司管理員** ：只能查看和管理自己的公司。 公司管理員也可以執行所有管理功能，包含增加管理員和使用者。公司管理員可以將使用者增加到 SPS 公司管理帳戶。(此角色為預設使用者角色。)

**SPS使用者** ：可存取已指派給他們的公司；不能履行行政職責。
...
1. 按一下「儲存」以完成使用者設定。
   ...
   如果使用者未收到歡迎電子郵件，請他們前往SPS登入頁面(https://s7sps1.scene7.com)，然後按一下「忘記我的密碼」。 已重設密碼並傳送新的電子郵件。如果使用者沒有收到電子郵件，也沒有在「垃圾郵件」資料夾中也發現，請聯絡「技術支援」。
