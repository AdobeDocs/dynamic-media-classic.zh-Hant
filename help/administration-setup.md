...
1. 按一下「**[!UICONTROL 增加]**」。
1. 輸入要添加的用戶或管理員的名稱和電子郵件地址，然後按一下&#x200B;**[!UICONTROL Next]**。
...
1. 如果要將用戶添加到組（如果要添加Media Portal用戶或參與者），請按一下&#x200B;**[!UICONTROL Next]**&#x200B;並添加用戶。
1. 按一下&#x200B;**[!UICONTROL 保存]**&#x200B;以完成用戶設定。

   系統會在儲存之後出現提示訊息，問您是否要將使用者增加至其他公司。如果要將用戶添加到公司，請按一下&#x200B;**[!UICONTROL 添加]**。
   ...
   新增媒體入口網站使用者時，您也可以前往「設定&#x200B;**** > **[!UICONTROL 應用程式設定]** > **[!UICONTROL 使用者管理]**」，然後按一下「上傳使用者清單」，並選取包含不超過500位使用者的。csv檔案。****
...
1. 按一下「**[!UICONTROL 設定]** > **[!UICONTROL 應用程式設定]** > **[!UICONTROL 管理設定]** > **[!UICONTROL 使用者管理]**」。
1. 在用戶清單中，選擇或取消選擇用戶名稱旁的&#x200B;**[!UICONTROL 活動]**&#x200B;選項。
...
1. 在表格中選擇顯示您要修改權限或存取權的公司的項目，然後按一下「管理公司」**[!UICONTROL 。]**
1. 選取使用者角色。
1. 如果您想要變更使用者的群組會籍（如果您正在編輯或新增Media Portal使用者或投稿者），請按一下「下一步」**[!UICONTROL 並編輯群組會籍。]**
1. 按一下&#x200B;**[!UICONTROL 「儲存」]**。
...
* **依群組篩選** -選取「依群 **[!UICONTROL 組]** 」選單，然後選擇選項，將清單縮小至群組中的使用者。

* **按用戶角色篩選** -選擇「按用 **[!UICONTROL 戶角色」]** 菜單，然後選擇一個選項，將清單限定給不同類型的用戶或管理員。

* **依欄位名稱篩選** -選取「 **[!UICONTROL 啟用依欄位篩選]**」。然後選擇&#x200B;**[!UICONTROL 按欄位名稱]**&#x200B;菜單，選擇用於篩選清單的列，然後選擇「篩選字元」菜單並選擇字母。 清單會依您選擇的字母列出其中一個欄。要查看完整清單，請取消選擇&#x200B;**[!UICONTROL Enable Filter By Field]**&#x200B;選項。

* **過濾掉無效用戶** -取消選 **[!UICONTROL 擇包含無效]**。搜尋結果只會顯示系統中的使用者。已從系統和您所管理的帳戶中刪除無效的使用者。

* **按欄標題排序** -按一下標題可依使用者的狀態、依名字、姓氏或電子郵件的字母順序、依使用者角色或有效／無效狀態來排序所有使用者。
...
Dynamic Media傳統管理員可以為所管理的公司產生頻寬、儲存空間和其他類型的報表。 這些報告可在「頻寬與儲存」頁面上取得。

要開啟此頁，請按一下「設定」**[!UICONTROL 「設定」]**>「個人設定」]**。**[!UICONTROL &#x200B;展開&#x200B;**[!UICONTROL 管理設定]** ，然後按一下&#x200B;**[!UICONTROL 頻寬和儲存]**。

### 報告類型 {#types-of-reports}

下表說明您可從「頻寬與儲存」頁面產生的報表：
...
* **Vnt Object Request**  —— 影像轉譯請求，可傳回在請求的暈映中具有指定物件的影像。

* **Vnt Info Request**  - Image rendering requests that return information abounted vignettes.
...
1. 選擇「**[!UICONTROL 設定]** > **[!UICONTROL 個人設定]**」。
1. 展開「管理設定」，然後按一下「頻寬和儲存」。****
1. 按一下標籤：**[!UICONTROL 頻寬]**、**[!UICONTROL 儲存]**、**[!UICONTROL 影像內容]**、**[!UICONTROL 網域]**、**[!UICONTROL 視訊串流]**&#x200B;或&#x200B;**[!UICONTROL 視訊內容]**。
...
* **在圖表或資料表格中檢視資料** -按一 **[!UICONTROL 下]** 圖表檢視圖表中的資料；按一 **[!UICONTROL 下「資]** 料檢視」在資料表格中檢視資料。

* **選擇報表簡報類型** -在「報表類型」功能表上，按一下「 **[!UICONTROL 摘要]**」、「每日 **** **** 」或「每月」，以匯總表單、依日或依月來組織資料。並非所有報告都提供此選項。

* **指定時段** -選擇選項以定義報表的時段，然後在定義時 **** 段後按一下更新：

* **預先定義的時段** -在「預先定義的報表」功能表上，選擇一個選項。例如，選擇「上月」可擷取前一個月的資料。

* **自訂時段** -在「預先定義報表」功能表上，按一下「自 **[!UICONTROL 訂」]**。然後，在&#x200B;**[!UICONTROL 開始月]**（或&#x200B;**[!UICONTROL 開始日期]**）功能表選擇日期，在月數(或月數（或天數）功能表選擇日期。 對於「網域」和「視訊內容報告」，您可以選擇特定開始和結束日期，以擷取報告資訊。
...
* **匯出報表資料** -在「資料」檢視中，視需要排序及排列資料。然後開啟&#x200B;**[!UICONTROL Export]**&#x200B;功能表，並選擇格式：**[!UICONTROL Tab分隔]**、**[!UICONTROL 逗號分隔]**&#x200B;或&#x200B;**[!UICONTROL HTML格式化]**。 資料會依您選擇的格式複製到剪貼簿。您現在可以將資料貼上試算表或應用程式。

* **列印報表** -按一下「列 **[!UICONTROL 印]**」，在「列印」對話方塊中選擇您要的選項，然後按一下「 **[!UICONTROL 確定]**」。

## 影像錯誤 {#image-errors}

Dynamic Media傳統型管理員可產生影像錯誤報告。 「影像錯誤」報告會針對您目前登入的公司，提供過去 24 小時內 20 個最常見的影像錯誤清單。若要產生「影像錯誤」報表，請執行下列動作：

1. 按一下「**[!UICONTROL Setup]** > **[!UICONTROL 個人設定]**」。
1. 展開「管理設定」，然後按一下「**[!UICONTROL 映像錯誤]**」。
1. (選擇性) 執行下列任一項作業:

   * 要按標題資訊對錯誤進行排序，請按一下標題。 根據預設，錯誤會按照出現次數由最高至最低排序。
   * 將游標移至錯誤的「回應」欄位，即可查看特定錯誤訊息。
   * 若要查看影像或反向連結網頁的連結，請將游標移至URL欄位或「反向連結」欄位上。
   * 若要將連結複製至實際影像，請按一下「URL複製URL ]**」。**[!UICONTROL &#x200B;您可以在瀏覽器視窗中貼上此連結，以前往影像並調查錯誤。
   * 若要將連結複製至反向連結網頁，請按一下「反向連結複製URL ]**」。**[!UICONTROL 

顯示的錯誤都屬於您目前所登入的公司。每個錯誤都包含下列資訊:

* **影像ID**  —— 違規影像的ID。

* **時間** -在過去24小時內，首次報告錯誤到上次報告錯誤的時間範圍。

* **Count**  —— 報告在映像上的錯誤數。

* **響應** -特定錯誤消息。錯誤為 4xx 或 5xx。

* **URL**  —— 列出Dynamic Media經典影像的URL。

* **反向連結** -指定初始請求來源之網站的URL。參照者可能是任何連結至該影像的網站。
