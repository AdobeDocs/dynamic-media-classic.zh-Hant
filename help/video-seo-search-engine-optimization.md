...
**視訊網站地圖** (Video Sitemap)告知Google視訊內容在網站上的確切位置和內容。 因此，您完全可以在 Google 上搜尋視訊。例如，視訊網站地圖可以指出視訊的執行時間和類別。如需視訊網站地圖的詳細資訊，請參閱https://www.google.com/support/webmasters/bin/answer.py?answer=80471。

**內容發佈者用來將媒體檔案饋送到Yahoo！的** mRSS（Media Really Simple Syndication，媒體簡易匯集）饋送 視訊搜尋。如需有關mRSS饋送的資訊，請參閱https://www.rssboard.org/media-rss。
...
**關閉** ：選擇此選項以停止產生視訊網站地圖和媒體RSS(mRSS)饋送。

在「自動/手動模式」下拉式清單中，選擇要自動或手動產生:

**自動模式** Dynamic Media Classic每天自動產生一個視訊網站地圖、媒體RSS(mRSS)饋送，或兩者。 選擇「標籤為發佈」選項，以自動標籤為發佈Dynamic Media Classic產生的XML檔案。

**當您在「視訊搜尋最佳化設定」畫面中按一下「產生」或「儲存並產生」時，Manual Mode** Dynamic Media Classic會產生視訊網站地圖、媒體RSS(mRSS)饋送，或兩者。 同時，也請選擇這些選項:

**沒有進一步設定** ：不會標籤為發佈所產生的XML檔案。

**標籤為發佈標籤** ：用於發佈生成的XML檔案。

**如果XML檔案未包含所有視訊的完整中繼資料資訊** ,「允許部分產生搜尋引擎」可能會拒絕該檔案。 即使部分視訊沒有中繼資料，此選項也會產生 XML 檔案。警告會登錄在「報告」畫面上。如果您要匯出 XML 檔案並手動處理遺失的資訊，請選擇此選項。
